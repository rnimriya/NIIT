...
import type { ChatRequest, ChatStreamEvent } from "@neet/types";
import { config, hasAnthropic, hasOpenAI } from "./config";
...
export class AiService {
  ...
  /**
   * Streams an AI tutor answer. Fallback chain:
   *   Claude (Opus for hard / Sonnet default) → OpenAI → deterministic mock.
   * Yields SSE-ready events; the final `done` event carries usage/cost meta.
   */
  async *streamTutor(req: ChatRequest): AsyncGenerator<ChatStreamEvent> {
  ...
  private async *viaClaude(
    ...
    const u = final.usage as { input_tokens?: number; output_tokens?: number; cache_read_input_tokens?: number };
    yield {
      type: "done",
      meta: {
        model,
